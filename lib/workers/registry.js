...
function create({ title, agentId, provider, model, systemPrompt, kind, cwd }) {
  ...
  db.prepare(`
    INSERT INTO workers (id, title, agent_id, provider, model, system_prompt, status, kind, cwd, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, 'idle', ?, ?, ?, ?)
  `).run(id, title || null, agentId || null, provider, model || null, systemPrompt || null, kind || "worker", safeCwd, now, now);
